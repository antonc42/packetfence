...
Let's say we have two roles: consultant and corporate_machine (defined above).

Now, we want to authenticate consultants and corporate machines using Active Directory (over LDAP), both using PacketFence's captive portal.
...
Test the connection and save everything. Using the newly defined source, any username that actually matches in the source (using the sAMAccountName) will have the consultant role and an unregistration date set to January 1st, 2020.
